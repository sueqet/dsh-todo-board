...
const TOOL_PARAMETERS = {
  ...
  properties: {
    action: {
      type: 'string',
      enum: ['list', 'add', 'done', 'reopen', 'note'],
      description: '要执行的操作。',
    },
  ...
  properties: {
    ...
    todos: {
      ...
      items: {
        ...
        required: ['id', 'title', 'dirLabel', 'mode', 'aiDone', 'verified'],
      },
    },
  },
...
export function apply(ctx) {
  ...
  async function dispatch(todo, sessionId) {
    const agents = ctx.get('agents')
    const target = agents !== undefined && sessionId !== '' ? agents.get(sessionId) : undefined
    if (target !== undefined) {
  ...
  if (tools !== undefined) {
    const definition = {
      name: TOOL_NAME,
      description:
        '读写跨会话 TODO 板。列表从上到下就是执行顺序（用户可拖动调整）。action=list 查看待办（默认当前工作目录），action=add 新增（追加到底部），action=done 标记 AI 已完成（左勾），action=reopen 撤销左勾，action=note 追加备注。',
        ...
        if (action === 'add') {
          ...
          return {
            message:
              ...
              todo.mode + '，追加到列表底部，id：' + todo.id + '）。',
            todos: [toolTodo(todo)],
          }
        }

        if (action === 'done' || action === 'reopen' || action === 'note') {
          const id = typeof input.id === 'string' ? input.id : ''
          const todo = find(id)
          ...
          save()
          const label =
            action === 'done' ? '已标记 AI 完成' : action === 'reopen' ? '已撤销 AI 完成' : '已更新备注'
