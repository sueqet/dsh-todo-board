...
export function apply(ctx) {
  ...
  function sendTo(todo, target, note) {
    if (target.status === 'running') {
      target.steer(noticeMessage(todoPrompt(todo), note + todo.title))
    } else {
      target.followup(noticeMessage(todoPrompt(todo), note + todo.title))
    }
    todo.dispatchedAt = Date.now()
  ...
  async function dispatch(todo, sessionId) {
    ...
    if (todo.mode === 'newSession') {
      ...
      if (bound !== undefined) return sendTo(todo, bound, 'TODO 接续：')
    ...
    if (target !== undefined) return sendTo(todo, target, 'TODO 接续：')
    todo.dispatchedAt = Date.now()
    touch(todo)
    afterChange()
    return {
