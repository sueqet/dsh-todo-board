  ...
  properties: {
    ...
    todos: {
      ...
      items: {
        ...
        required: ['id', 'title', 'dirLabel', 'mode', 'schedule', 'imageCount', 'aiDone', 'verified'],
      },
    },
  },
...
export function apply(ctx) {
  ...
  async function attachToWorkspace(sessionId, dirPath) {
    ...
    try {
      ...
      ctx.logger?.warn?.('todo-board: could not group the new session: ' + errText(err))
      return 'failed'
    }
  }
  ...
  if (tools !== undefined) {
    const definition = {
      name: TOOL_NAME,
      description:
        '读写跨会话 TODO 板。列表从上到下就是执行顺序（用户可拖动调整）。action=list 查看待办（默认当前工作目录），action=add 新增（追加到底部，可用 images 附主机上的图片文件），action=done 标记 AI 已完成（左勾），action=reopen 撤销左勾，action=note 追加备注，action=schedule 设置/取消定时执行时间。带 schedule 的待办到点后才会被派发。',
